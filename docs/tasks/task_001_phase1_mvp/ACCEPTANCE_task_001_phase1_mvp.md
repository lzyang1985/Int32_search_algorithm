---
title: 验收检查 — Phase 1 MVP (Path A 单路径)
status: Archived
...
archive_target: "docs/decisions/phase1_mvp_acceptance.md"
reviewer: Human
...
```
⚠️ **注意**: Windows 环境下 AVX2 路径显著慢于标量（0.49x-0.55x speedup，vs Linux 5.26x）。此现象不影响 Linux 生产环境，但需独立调查跟踪。

**根因假说（meeting_004 讨论，按可信度从高到低排序）**：
1. **`__builtin_popcount` 软件模拟**（Algorithm 提出，最可能根因）：GCC 15.2.0 MinGW 在 `-mavx2` 下未正确启用 TARGET_POPCNT，`__builtin_popcount` 走 `__popcountsi2` 库调用而非 `popcnt` 硬件指令，每次迭代额外 ~30-50 cycles
2. **GCC/MinGW 代码生成质量**（Backend/Architect）：YMM 寄存器溢出到栈、循环展开策略差等编译器版本特有问题
3. **Benchmark 方法论偏差**（见偏差 D-07）：当前 AVX2 走完整 API 路径，Scalar 是内联裸循环，非公平比较
4. **系统层面因素**（Backend）：频率缩放（Turbo ramp-up 时序）、RDTSC 无序列化、跨核迁移等

调查由独立子任务 `task_002_windows_avx2_investigation`（meeting_004 D-033）跟踪，与 Phase 2 并行推进，不阻塞 Linux 交付。
...
| SR-05 | ASan/UBSan 零告警 | ✅ Linux 验证通过 | Phase 2 task_003/TODO-02 确认：Intel Xeon Gold 6152, GCC 11.4, Ubuntu 22.04, ASan/UBSan 全量测试零告警 PASS |
...
| D-07 | Benchmark 方法论偏差 | Medium | 📝 已记录 | AVX2 走 API vs Scalar 内联裸循环非公平比较 + RDTSC 无序列化 |
...
### 5.8 D-07 详情：Benchmark 方法论偏差 [Medium] ← meeting_004 D-030 新增

**描述**: 当前 benchmark 中 "AVX2" 路径走完整 API 调用链（`int32_search_find` → CPU 检测 → NULL 检查 → `search_avx2_find`，约 6 次分支 + 2 次跨编译单元函数调用），而 "Scalar 基线" 是裸循环内联 `bsearch`/标量二分（含 `__rdtsc()` 也在内联循环内），两者非公平比较。API 层开销估算 ~24-38 cycles/query。

此外，计时方法也存在 3 个系统性问题：
1. **频率缩放**：AVX2 先跑时 CPU 仍在 Turbo ramp-up 阶段，Scalar 后跑时 Turbo 已激活，系统性地偏向 Scalar
2. **RDTSC 无序列化**：`__rdtsc()` 无 `lfence`/`cpuid` 序列化屏障，乱序执行污染测量
3. **跨核迁移**：Windows 调度器可能将 benchmark 迁移到不同核心，引入噪声

**修复方案（meeting_004 D-031 决议）**:
1. 增加 5 组公平对照：API+AVX2 / API+Scalar / Raw AVX2 / Raw Scalar / Inline Scalar
2. 使用 `__rdtscp()` + `_mm_lfence()` 替代 `__rdtsc()`
3. 增加实质性 warmup（100ms+ 循环预热）
4. 支持 `INT32SEARCH_BENCH_SEED` 环境变量实现可重现 benchmark
5. 增加 CPU 特性打印（型号、频率、AVX2 检测状态）

**执行时机**: Phase 2 开发中同步改进（P1），不阻塞 Phase 2 核心交付。

**严重程度**: Medium — 影响基准数据的正确解读（尤其 Windows 对比），但不影响功能性正确。Linux 生产环境 5.26x 结论不受影响（Scalar 也是 API 路径时差距可能更大）。
